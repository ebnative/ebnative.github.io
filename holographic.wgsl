// Adapted from vgpu holographic-card, Vercel Inc., MIT. See vgpu-LICENSE.txt.
// Material functions preserved; panel UV mapping and full-bleed output adapted.
struct Params {
  resolution: vec2f,
  tilt: vec2f,
  pointer: vec2f,
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let screen = (uv - 0.5) * vec2f(1.28, 1.82);
